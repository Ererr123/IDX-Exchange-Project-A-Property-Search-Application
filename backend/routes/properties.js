...
// whitelist uses actual sql column names from rets_property
...
router.get('/:id/openhouses', async (req, res) => {
    try {
        ...
        // verify property exists before querying open houses
...
router.get('/', async (req, res) => {
    try {
        ...
        // validate sort params against whitelist
        const { sortBy, sortOrder } = req.query;

        if (sortBy && !SORT_WHITELIST[sortBy]) {
        ...
        // build order by clause using whitelisted column name
        const orderColumn = sortBy ? SORT_WHITELIST[sortBy] : 'L_ListingID';
        const orderDirection = sortOrder?.toLowerCase() === 'asc' ? 'ASC' : 'DESC';
        const orderBy = `ORDER BY ${orderColumn} ${orderDirection}`;
        ...
        const dataQuery = `SELECT L_ListingID, L_Address, L_City, L_State, L_Zip, L_SystemPrice, L_Keyword2, LM_Dec_3, LM_Int2_3, L_Photos FROM rets_property ${where} ${orderBy} LIMIT ? OFFSET ?`;
